...
var skillsProgressAnimation = function() {
	var $section = $('#skills-section');
	var $bars = $section.find('.skill-progress-track span');

	if (!$section.length || !$bars.length) {
		return;
	}

	$bars.each(function() {
		var $bar = $(this);
		var targetWidth = this.style.width || $bar.attr('data-width') || '0%';
		$bar.attr('data-width', targetWidth);
		$bar.css('width', '0%');
	});

	$section.waypoint(function(direction) {
		if (direction === 'down' && !$section.hasClass('skills-animated')) {
			$section.addClass('skills-animated');

			$section.find('.skill-progress-track span').each(function(index) {
				var $bar = $(this);
				var target = $bar.attr('data-width') || '0%';
